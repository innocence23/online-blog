...
@section('title', '字典栏目管理')

@section('content_header')
    <h1>字典栏目管理</h1>
    <ol class="breadcrumb">
        <li class="active">字典栏目管理</li>
    </ol>
@stop
...
@section('css')
    <link rel="stylesheet" href="{{asset('/vendor/plugins/angular-tree-dnd-master/dist/ng-tree-dnd.css')}}">
@stop

@section('js')
    <script src="{{ asset('/vendor/plugins/angular-tree-dnd-master/dist/ng-tree-dnd.js') }}" type="text/javascript"></script>
    <script>
        angular.module('myModule', ['ntt.TreeDnD'])
            .controller('myController', function ($scope, $http, $TreeDnDConvert, $timeout) {
                ...
                $scope.col_defs = [
                    {
                        ...
                        displayName: '状态',
                        titleClass:  'text-center',
                        cellTemplate:'<span ng-if="node.status == 0" class="label label-warning">禁用</span>' +
                        '<span ng-if="node.status == 1" class="label label-info">启用</span>'
                    },
                    {
                        titleClass:  'text-center',
                ...
                refresh();
                function refresh() {
                    $http.get("/admin/dict/category/lists")
                ...
                $scope.save = function (modalstate, id) {
                    var url = "{{route('dict.category.store')}}";
                    ...
                    $http({
                        ...
                        if (modalstate === 'edit') { //修改时刷新当前页
                            ...
                            $("#table").bootstrapTable('refresh', {url: "{{route('dict.category.lists')}}"});
                        }
                        refresh();
